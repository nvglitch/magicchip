...
const megaMenuProducts: Record<string, MegaMenuProduct[]> = {
  ...
  'ai-mini-pc': [
    ...aiCatalog.map(({ id, name, tagline, image }) => ({ id, name, tagline, image })),
...
export default function Navbar() {
  ...
  const getDropdownContent = (): Record<string, DropdownItem[]> => ({
    ...
    contact: [
      { title: t.navDropdown?.contact?.email || 'Email', description: t.navDropdown?.contact?.emailDesc || 'Send us an email', href: 'mailto:klauslee@szmagicchip.com', icon: Mail, external: true },
                  ...
                  {item.dropdown && activeDropdown === item.label && (
                    item.href === '/products' ? (
                        ...
                        <div className="grid gap-5 md:grid-cols-[190px_1fr] lg:grid-cols-[230px_1fr]">
                          ...
                          <div className="min-w-0">
                            ...
                            {(activeMegaCategory.id === 'industrial-mini-pc' || activeMegaCategory.id === 'firewall-mini-pc' || activeMegaCategory.id === 'commercial-mini-pc') ? (
                              ...
                              <AiFamilyCards products={activeMegaCategory.products} />
                            )}
                          </div>
                        </div>
                      </motion.div>
                      ...
                      >
                        <div className="mb-5 flex items-center justify-between border-b border-slate-200 pb-4">
                          <div>
                            <h2 className="text-2xl font-bold text-slate-950 lg:text-3xl">{t.nav.solutions}</h2>
                          </div>
