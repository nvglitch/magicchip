...
项目实现以代码和内容数据为准；持续工作规范见 `AGENTS.md`，专题说明位于 `docs/`。

## 产品册子更新来源

后续产品上新及参数更新统一读取 `D:\本地资源库\电脑相关\电脑册子归档\ver2`，用户无需再将新册子复制到 `public/assets`。收到“检查变动并添加”时，按来源指纹和当前网站内容核对后完成本地更新，仍由用户手动发布。

具体流程见 [产品册子来源与更新规范](docs/产品册子来源与更新规范.md)，来源观察记录见 [册子清单](docs/brochure-source-manifest.json)。首次观察记录不代表已同步到网站。
